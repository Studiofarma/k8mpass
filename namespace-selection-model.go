...
func (n NamespaceSelectionModel) Update(msg tea.Msg) (NamespaceSelectionModel, tea.Cmd) {
	...
	case namespace.ListMsg:
		...
		cmds = append(cmds, namespace.Route(n.namespaces.SetItems(items)))
	...
	case namespace.AddedMsg:
		cmds = append(cmds, namespace.Route(n.namespaces.InsertItem(0, msg.Namespace)))
		ns := n.namespaces.Items()
		sort.SliceStable(ns, func(i, j int) bool {
			return ns[i].FilterValue() < ns[j].FilterValue()
		})
		cmds = append(cmds, namespace.Route(n.namespaces.SetItems(ns)))
		cmds = append(cmds, n.messageHandler.NextEvent)
		cmds = append(cmds, namespace.Route(n.namespaces.NewStatusMessage(fmt.Sprintf("ADDED: %s", msg.Namespace.K8sNamespace.Name))))
	case namespace.ModifiedMsg:
		var idx = namespace.FindNamespace(n.namespaces.Items(), msg.Namespace)
		cmds = append(cmds, namespace.Route(n.namespaces.SetItem(idx, msg.Namespace)))
		cmds = append(cmds, n.messageHandler.NextEvent)
	case namespace.RemovedMsg:
		var idx = namespace.FindNamespace(n.namespaces.Items(), msg.Namespace)
		n.namespaces.RemoveItem(idx)
		cmds = append(cmds, n.messageHandler.NextEvent)
		cmds = append(cmds, namespace.Route(n.namespaces.NewStatusMessage(fmt.Sprintf("REMOVED: %s", msg.Namespace.K8sNamespace.Name))))
	...
	case namespace.ReloadExtensionsMsg:
		...
		cmds = append(cmds, namespace.Route(n.namespaces.SetItems(items)))
		cmds = append(cmds, namespace.Refresh())
		cmds = append(cmds, namespace.Route(n.namespaces.NewStatusMessage("Reloaded")))
	case namespace.RoutedMsg:
		model, cmd := n.namespaces.Update(msg.Embedded)
		n.namespaces = model
		cmds = append(cmds, namespace.Route(cmd))
	case tea.KeyMsg:
